...
export function handleSchedulingSnapshot(event: SnapshotSchedule): void {
    let oldSnapshot = Snapshots.load(event.params.oldTime.toHexString().concat('-').concat(event.transaction.hash.toHexString()));
    if(oldSnapshot==null){
        let ot = event.params.oldTime.toHexString();
...
export function handleUnschedulingSnapshot(event: SnapshotUnschedule): void {
    let newSnapshot = Snapshots.load(event.params.time.toHexString().concat('-').concat(event.transaction.hash.toHexString()));
    if(newSnapshot==null){
        let timeset = event.params.time.toHexString();
